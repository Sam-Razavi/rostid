...
import { listAddresses, createAddress, updateAddress, deleteAddress, listShippingRates } from './shipping.service';
...
router.post('/addresses', asyncHandler(async (req: Request, res: Response) => {
  const address = await createAddress(req.user!.userId, req.body as Parameters<typeof createAddress>[1]);
  res.status(201).json({ data: address, message: 'Address created' });
}));

router.patch('/addresses/:id', asyncHandler(async (req: Request, res: Response) => {
  const address = await updateAddress(req.user!.userId, req.params.id, req.body as Parameters<typeof updateAddress>[2]);
  res.json({ data: address, message: 'Address updated' });
}));

router.delete('/addresses/:id', asyncHandler(async (req: Request, res: Response) => {
